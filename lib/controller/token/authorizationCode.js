...
module.exports = function(oauth2, client, sCode, redirectUri, pCb) {
    ...
    async.waterfall([
        ...
        function(cb) {
            oauth2.model.refreshToken.fetchByUserIdClientId(oauth2.model.code.getUserId(code), oauth2.model.code.getClientId(code), function (err, token) {
                if (err) {
                    ...
                    oauth2.logger.debug('No existing refresh token, creating one.');
                }
                cb();
            });
        },

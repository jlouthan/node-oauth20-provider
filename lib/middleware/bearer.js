...
module.exports = function (req, res, next) {
    ...
    if (req.headers.authorization) {
        ...
        if (!pieces || pieces.length !== 2)
            return response.error(req, res, new error.invalidRequest('Wrong authorization header'));
        // Only bearer auth is supported
        if (pieces[0].toLowerCase() != 'bearer')
            return response.error(req, res, new error.invalidRequest('Unsupported authorization method header'));
        token = pieces[1];
    ...
    // Not found
        ...
        return response.error(req, res, new error.invalidRequest('Bearer token not found'));
    ...
    req.oauth2.model.accessToken.fetchByToken(token, function(err, object) {
        if (err)
            ...
            response.error(req, res, new error.unauthorized('Token not found or expired'));
        }
        else if (!req.oauth2.model.accessToken.checkTTL(object)) {
            response.error(req, res, new error.unauthorized('Token already expired'))
        }
        else {
